...
from src.models.common import Joystick
                ...
                for fd in r:
                    try:
                        ...
                        if eType == self.EVENT_AXIS:
                            self.queue.put(events.JoystickEvent(events.AXIS_EVENT, time, fd, number, value))
                        elif eType == self.EVENT_BUTTON:
                            self.queue.put(events.JoystickEvent(events.BUTTON_EVENT, time, fd, number, value))
    ...
    def run(self):
        ...
        while(self.consuming):
            ...
            if e.eventType() == events.NEW_EVENT:
                ...
                t = Thread(target=self.processEvent, args=(e,))
                t.start()
...
class JoystickHolder(object):
    ...
    """
    def __init__(self):
        super(JoystickHolder, self).__init__()
        self.joylist = {}
        self.queue = Queue()
